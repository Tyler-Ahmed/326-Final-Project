...
#create a function that prompts the user to go back to the menu screen
...
#Allow user to save their score to a seperate file
...
#Allow the user an option to save their own question and add it to a test.
    def add_new_question(self, quizzes, quiz_index):
        ...
        # Create a new quiz with the added question
        new_quiz = Quiz(f"{self.subject}(changed)", self.difficulty)
        new_quiz.add_question(new_question, new_answer)
        quizzes.append(new_quiz)  # Add the new quiz to the quizzes list

        print("New question added successfully to the new quiz!")

        # Return to the main menu
        main()


#This is the main method. Create method to run the test and allow users to input answers and check their score.
    def run_quiz(self, quiz):
        ...
        print(f"Your percentage score is: {percentage_score:.2f}%") #User will be presented with their overall score for the test
        
        name = input("Enter your name: ")#input name after taking test to save score
...
#Have an option to retake the test if you didnt get a 100%
...
#Have an option to display the scores after taking a test or from the main menu
...
#Option to reset the scores saved from your file
...
#Creat some somple data to input into the tests for each subject
def sample_quizzes():
...
#This is the main function that will propt the main menu. After users can select what they whant to do which will prompt certain methods to run
def main():
    ...
    if choice == "1":
        ...
        if quiz_choice.isdigit():
            quiz_choice = int(quiz_choice)
            if 1 <= quiz_choice <= 6:
                quizzes[quiz_choice - 1].add_new_question(quizzes, quiz_choice - 1)  # Pass the quizzes list and index
            else:
                print("Invalid quiz number. Please enter a number between 1 and 6.")
        else:
            print("Invalid input. Please enter a number between 1 and 6.")
